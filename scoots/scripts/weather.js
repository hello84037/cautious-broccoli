document.addEventListener('DOMContentLoaded', function() {
    ...
    function displayCurrentTemp(data) {
        ...
        let iconDesc = data.weather.map(item => {
    ...
    function maxTemp(dailyForecasts) {
        ...
        dailyForecasts.forEach(forecastDay => {
            // Get the maximum temperature for the current day
            const maxTempOfDay = forecastDay.reduce((maxTemp, forecast) => {
                if (forecast.main.temp > maxTemp) {
                    maxTemp = forecast.main.temp;
                    maxTempToday = forecast.dt_txt; // Update maxTempToday if this temperature is higher
                }
                return maxTemp;
            }, -Infinity);

            // Update maxTemperature if the maximum temperature of the current day is higher
            if (maxTempOfDay > maxTemperature) {
                maxTemperature = maxTempOfDay;
            }
        });

        maxTempElement.textContent = `Today's High Temperature: ${Math.round(maxTemperature)}°F`;
    }
    ...
    function displayForecastTemps(dailyForecasts) {
        ...
        const forecastForTomorrow = dailyForecasts.find(forecast => {
            const forecastDate = new Date(forecast[0].dt * 1000);
            return (
        ...
        if (forecastForTomorrow) {
            const forecastAt15 = forecastForTomorrow.find(entry => entry.dt_txt.includes("15:00:00"));
            ...
            if (forecastAt15) {
                ...
                let forecastDate = new Date(forecastForTomorrow[0].dt * 1000);
